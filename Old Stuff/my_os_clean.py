...
class Resource:
    ...
    def request(self, req_user):
        for user, time in self.users.items():
            if user.id == req_user:
                self.status = RESOURCE_IN_USE
                self.current_user, self.time_left = user, time
...
def main():
    resources = [Resource(i+1) for i in range(30)]
    users = [User(i+1) for i in range(30)]
    ...
    for user in users:
        ...
        for i in resource_indices:
            ...
            user.add_resource(resource, time)
            resources[i].allocate(user, time)
        ...
        for user in users: 
            ...
            for resource in resources: # when resource is  available
                ...
                if resource == user.requested_resource and resource.status == RESOURCE_AVAILABLE and user.status == USER_AVAILABLE:
                    ...
                    resource.request(user.id) # updates status, current user, and time left in resource
                    break
